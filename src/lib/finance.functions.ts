...
export const getFinanceChartData = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    return Object.values(monthlyData).sort((a, b) => {
      ...
      const monthIndexA = mA ? monthNames.indexOf(mA) : -1;
      const monthIndexB = mB ? monthNames.indexOf(mB) : -1;
      const dateA = new Date(`20${yA || '26'}-${String(monthIndexA !== -1 ? monthIndexA + 1 : 1).padStart(2, '0')}-01`);
      const dateB = new Date(`20${yB || '26'}-${String(monthIndexB !== -1 ? monthIndexB + 1 : 1).padStart(2, '0')}-01`);
